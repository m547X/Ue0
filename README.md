...
### اختيار جراج معيّن من داخل اللعبة

تبويب **الجراجات** في الواجهة يعرض كل الجراجات الموجودة في الملف، ومع كل جراج
رقمان: الأصفر = السيارات بدون صورة، والرمادي = إجمالي سيارات الجراج.
اضغط على الجراجات التي تريدها ثم **تطبيق الاختيار** — وسيقتصر التصوير عليها.
بدون أي اختيار = كل الجراجات. الاختيار يتجاوز `GarageFile.garages` في الكونفق.

### التحكم اليدوي بالتصوير

مفيد إذا كانت بعض السيارات غير موجودة أو الصورة تطلع خطأ.

```lua
-- Config.lua
Config.ManualCapture = false   -- أو فعّله من تبويب "التصوير" في الواجهة
```

عند تفعيله يقف عند كل سيارة وينتظر منك:

| المفتاح | الوظيفة |
|---------|---------|
| `Enter` | صوّر وانتقل للسيارة التالية |
| `Space` | تخطَّ هذه السيارة بدون تصوير |
| `→` `←` | تدوير السيارة |
| `Backspace` | إنهاء الجلسة |

**والتحكم بالكاميرا أثناء الوقوف على السيارة:**

| المفتاح | الوظيفة |
|---------|---------|
| `W` `A` `S` `D` | تحريك الكاميرا |
| الماوس | اتجاه النظر |
| عجلة الماوس | زوم (FOV) |
| `Q` / `E` | رفع / خفض الكاميرا |
| `Shift` | حركة أسرع |
| `R` | إرجاع كاميرا المكان الأصلية |

تعديل الكاميرا **يبقى للسيارات التالية في نفس الجلسة**، ولا يغيّر إعدادات المكان
المحفوظة — لحفظها بشكل دائم استخدم محرر الكاميرا من تبويب الإعدادات.
سرعات الحركة والزوم تُؤخذ من `Config.CameraEditor` الموجود أصلاً.

السيارة المتخطّاة لا تُحتسب في العداد، واختيارك من الواجهة يُحفظ فيبقى بعد
إعادة تشغيل اللعبة.
