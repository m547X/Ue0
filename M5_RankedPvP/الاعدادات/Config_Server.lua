...
-- ============================================================================
-- الكونسول — وش يطلع بشاشة السيرفر (cmd) وبـ F8
-- ============================================================================
-- السكربت كان يكتب أسطر معلومات بالكونسول وقت التشغيل ووقت اللعب: بدأ، وشغّل
-- قيم بوتات، ولاعب اشترى شي من المتجر ... إلخ. صارت مطفية.
--
-- الأخطاء شي ثاني تماماً. لو طفّيتها وصار خلل — ما ينحفظ رانك، أو قاعدة
-- البيانات ترفض كتابة — السكربت بيسكت وأنت ما تدري ليش، وأنا ما أقدر أساعدك
-- لأن ما فيه شي أقرأه. خلّها true.
Config.Console = {
    info   = false,   -- أسطر المعلومات العادية (مطفية)
    errors = true     -- الأخطاء (خلّها مشغّلة)
}
...
-- ============================================================================
-- ما فيه شي تعدّله هنا. إعدادات الداتابيس (إنشاء الجداول، الكتابة المجمّعة،
-- الكاش، حجم الصفحة) صارت جوّه السكربت بقيم مضبوطة أصلاً، عشان ما تنعبّى
-- عليك بأرقام تغييرها يضر أكثر مما ينفع.
--
-- ⚠ لو كنت حاطط Config.Database بنسخة قديمة، السكربت يقراها ويحترمها.
...
Config.Permissions      = {
    -- ------------------------------------------------------------------
    -- الصلاحية العليا — مفتاح عام
    -- الي يملكها عنده **كل شي** بالسكربت بدون ما يحتاج ولا صلاحية ثانية:
    -- كل أوامر لوحة الإدارة، ومحرّر الشاشات والتماثيل (/pvpboard)، وفتح
    -- الواجهة، وإنشاء الغرف، والمشاهدة، ورؤية الـ MMR المخفي.
    -- لا تعطيها إلا لصاحب السيرفر.
    -- ------------------------------------------------------------------
    superAdmin     = 'pvp.all',

    -- امتلاك صلاحية الإدارة العامة كمان يفتح كل شي، نفس الي فوق.
...
--
-- ---- صور الشارات (اختيارية) -------------------------------------------
-- بشكل افتراضي الشارة **مرسومة** بالواجهة (شكل سداسي بلون الدرجة)، وتشتغل
-- بدون أي ملف. لو عندك صور شارات، فيه طريقتين وما تحتاج الاثنتين:
--
-- ١) بالاسم والمكان: حط الصور تحت
--        Files/ui/img/patents/<الدرجة>/<الدرجة>_<الرقم>.png
--    يعني Diamond II صورتها patents/Diamond/Diamond_2.png، والدرجة الي ما
--    فيها تقسيمات (مثل Radiant) تكون patents/Radiant/Radiant.png.
--    الواجهة تدوّرها بنفسها، ما تحتاج تكتب شي هنا.
--
-- ٢) أو تكتب المسار بنفسك بحقل `img` بالسطر نفسه، وهذا يغلب الأول ويخليك
--    تسمّي المجلدات باللي تبي:
--
--        { id = 22, tier = 'IMMORTAL', ... , img = 'patents/Ruby/Ruby_1.png' },
--
--    المسار نسبة لـ Files/ui/img/ ، والصور لازم تكون داخل مجلد img عشان
--    قائمة `files` بملف fxmanifest.lua تشحنها (**/*.png موجودة أصلاً).
--
-- الصورة تنحط **فوق** الشكل المرسوم، فأي درجة ما لقت صورتها تبقى بشكلها
-- الأصلي بدل ما تطلع فاضية — تقدر تعبّي المجلد على راحتك.
...
Config.MapVote          = {
    ...
    -- ---- تصويت السلاح ----------------------------------------------------
    -- ينعرض بنفس شاشة تصويت الماب وبنفس المؤقت. اللي يفوز يصير سلاح القيم
    -- لكل اللاعبين. إذا انطفى، القيم ينزل بسلاح الطور الافتراضي من
    -- Config.Loadouts.
    weapons     = {
        enabled = true,
        -- كم سلاح ينعرض للاختيار. يتسحبون من Config.WeaponPresets عشوائياً.
        -- حط 0 عشان يعرض كل الأسلحة الموجودة.
        options = 4,
        -- أسلحة تنعرض دائماً بالتصويت مهما كان السحب العشوائي. آيديات من
        -- Config.WeaponPresets. مثال: { 'assault_rifle', 'sniper' }
        always  = {},
        -- أسلحة ما تنعرض بتصويت الرانك أبداً
        exclude = {}
    },

    -- ---- تصويت قاعدة القتل ----------------------------------------------
    -- خياران: "الرأس فقط" يعني ما يموت إلا بطلقة رأس، و"الجسم كامل" يعني
    -- القتل العادي. اللي يفوز ينطبق على القيم كله.
    headshot    = {
        enabled = true,
        -- الخيار الي ينحط إذا ما صوّت أحد، أو إذا انطفى التصويت:
        --   false = الجسم كامل (العادي)
        --   true  = الرأس فقط
        default = false
    }
}
...
Config.Match            = {
    ...
    -- ---- التريد (الذبح المتبادل) ----------------------------------------
    -- لما يطيح آخر لاعب بفريق، السيرفر ما ينهي الراوند على طول — ينتظر هذي
    -- المدة (ملي ثانية) عشان الرصاص الي كان طاير يوصل. إذا طاح الي قتله
    -- كمان خلال المدة، الراوند **تعادل** وينعاد، والقتلة الثانية تنسجّل
    -- كاملة (موت وقتل و K/D) بدل ما تنرمى.
    --
    -- بدونها الي يوصل خبر موته للسيرفر أول يخسر الراوند — يعني **البنق**
    -- هو الي يقرر، مب الاشتباك. وبطارية الحالتين: اللاعب الثاني ما ينحسب
    -- عليه موت ولا ينحسب للي قتله قتلة.
    --
    -- ٣٠٠ مناسب لسيرفر بنقات طبيعية. ارفعها لو لاعبينك بنقهم عالي.
    -- صفر = تطفيها وترجع للسلوك القديم (أول واحد يموت يخسر).
    -- ⚠ هذي تأخّر ظهور بانر نهاية الراوند بنفس المدة تقريباً.
    tradeWindow          = 300,
...
-- ============================================================================
-- ١٥أ. إرسال كود الغرفة للشات
-- ============================================================================
-- زر الشات الي جنب كود الغرفة بواجهة الغرف الخاصة.
--
-- الإرسال من السيرفر مو من الكلنت، عشان سببين: الرسالة توصل الكل، والكلنت
-- ما يقدر يزوّر نصها ولا يتخطى منع التكرار.
--
-- منع التكرار: الضغطة الوحدة ترسل **مرة وحدة فقط** مهما دبّلت الضغط، وبعدها
-- اللاعب ما يقدر يرسل مرة ثانية إلا بعد cooldown ثانية. والسيرفر يتأكد أنك
-- فعلاً داخل غرفة قبل ما يرسل كودها.

Config.RoomCodeChat     = {
    enabled  = true,

    -- شكل الاستدعاء الي يستخدمه السيرفر:
    --     TriggerEvent('chatMessage', prefix, color, message)
    -- إذا سيرفرك عنده ريسورس شات ثاني يبي شكل مختلف، غيّر event تحت.
    event    = 'chatMessage',

    -- الاسم الي يطلع قبل الرسالة
    prefix   = '#system |',

    -- لون الاسم { أحمر, أخضر, أزرق }
    color    = { 255, 0, 0 },

    -- نص الرسالة. %s الأولى = اسم اللاعب، %s الثانية = الكود.
    -- تقدر تستخدم أكواد ألوان الشات: ^1 أحمر ^2 أخضر ^3 أصفر ^0 رجوع للعادي.
    message  = 'اللاعب [^3 %s ^0] فتح غرفة رانك — الكود : [^2 %s ^0]',

    -- ثواني قبل ما يقدر نفس اللاعب يرسل مرة ثانية
    cooldown = 60,

    -- تحذير للاعب إذا ضغط وهو لسه بالمهلة. false يخليه ما يشوف شي.
    tellOnCooldown = true
}

-- ============================================================================
-- ١٥ب. لوحة المتصدرين الي تنعرض باللعبة
-- ============================================================================
-- هذا القسم يقرر **البيانات** فقط: أي لادر، كم لاعب، وكل كم يتحدّث.
-- أما **مكان** الشاشات ومكان وقوف التوب ٣ فهي بـ Config_Client.lua تحت
-- Config.WorldBoard — لأنها أماكن ومناظر، ما فيها شي خاص بالسيرفر.
--
-- السيرفر يسحب القائمة **مرة وحدة** كل refresh ويرسلها للكل. يعني مهما كان
-- عدد اللاعبين الواقفين قدام الشاشة، الاستعلام واحد. وما يسحب أصلاً إذا ما
-- فيه ولا لاعب أونلاين.

Config.WorldBoard       = {
    enabled = true,

    -- أي لادر تعرضه الشاشة. nil = اللادر الافتراضي من Config.RankPools.
    -- مثال: '5v5' أو '1v1'
    mode    = nil,

    -- كم لاعب ينعرض بالشاشة (الحد الأعلى ٢٥)
    top     = 10,

    -- true : ما ينعرض إلا اللاعبين الي خلّصوا مباريات التحديد.
    -- false: ينعرض الكل من أول يوم.
    --
    -- خلّها false. لأنه لو true وسيرفرك جديد وما أحد خلّص التحديد بعد، اللوحة
    -- تطلع فاضية تماماً — وتحسب إنها خربانة وهي شغالة وما عندها أحد تعرضه.
    requirePlacement = false,

    -- كل كم ثانية تتحدّث القائمة. ٣٠٠ = كل خمس دقايق. ما تخليها قليلة
    -- بدون سبب — كل تحديث استعلام على الداتابيس.
    refresh = 300
}
...
Config.CustomGames      = {
    ...
    defaults          = {
        mode            = '5v5',          -- الطور
        map             = 'arena1',       -- الماب (لازم يكون آيدي موجود بـ Config.Maps)
        ...
        -- البداية التلقائية: إذا true، القيم يبدأ لحاله أول ما تمتلئ الأماكن
        -- بالنسبة تحت — بدون ما المضيف يضغط شي. خلّه false عشان المضيف هو
        -- الي يبدأ بزر "ابدأ" لما يكون جاهز. المضيف يقدر يغيّره من إعدادات
        -- الغرفة، وهذا الي يبدأ عليه.
        autoStart       = false,
