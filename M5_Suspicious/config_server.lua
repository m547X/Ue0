...
--  vRP  |  جداول vRP
-- ============================================================
-- يقرأ النظام user_id مباشرة من جداول vRP بدل استدعاء الـ Proxy،
-- لأن Proxy الخاص بـ vRP 0.5 يعيد نتيجة الاستدعاء السابق إذا فشل
-- (متغيّر proxy_rdata مشترك)، وهذا يعني ربط لاعب بحساب لاعب آخر.
-- عدّل أسماء الجداول هنا فقط لو كان لديك fork بأسماء مختلفة.

Config.VRP = {
    IdentifiersTable  = "vrp_user_ids",
    IdentifierColumn  = "identifier",
    UserIdColumn      = "user_id",

    IdentitiesTable   = "vrp_user_identities",

    -- فحص سلامة الـ Proxy قبل كل تحقق صلاحية. صلاحية وهمية لا يملكها أحد:
    -- لو رجعت true فهذا يعني أن الـ Proxy يعيد بيانات قديمة، فتُرفض العملية.
    ProbePermission   = "m5.__integrity_probe__",
}
